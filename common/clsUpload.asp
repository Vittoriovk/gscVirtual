...
' ------------------------------------------------------------------------------
	...
	End Property
' ------------------------------------------------------------------------------	
	Public Property Get NameOf(ByRef avKeyIndex)
		Dim lnIndex
		lnIndex = IndexOf(avKeyIndex)
		if lnIndex = -1 Then Exit Property
		NameOf = oFields(lnIndex).FieldName
	End Property
' ------------------------------------------------------------------------------
...
' ------------------------------------------------------------------------------
	Private Sub Class_Initialize()
		...
		If Session("UtenteLogin_IdUtente") =26 Then
		response.write "<br>lnByteCount: "&lnByteCount
		
		End If 
		lnBytes = Request.BinaryRead(lnByteCount)
		' If Session("UtenteLogin_IdUtente") =26 Then
		' response.write "<br>CStrU(lnBytes): "&CStrU(lnBytes)
		' End If
