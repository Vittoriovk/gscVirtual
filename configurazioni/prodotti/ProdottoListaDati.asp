<%
  NomePagina="ProdottoListaDati.asp"
  titolo="Prodotto : dati aggiuntivi"
...
<%
  ...
  if Oper="CALL_INS" or Oper="CALL_UPD" then 
     ...
     if kk <> "" then 
        ...
        Session("swap_PaginaReturn")  = "configurazioni/prodotti/ProdottoListaDati.asp"
        response.redirect virtualPath & "configurazioni/prodotti/ProdottoDatoModifica.asp"
        response.end 
     end if 
  End if 
  if Oper="CALL_INS_TEC" or Oper="CALL_UPD_TEC" then 
     ...
     if kk <> "" then 
        ...
        Session("swap_OperTabella")   = Oper
        Session("swap_PaginaReturn")  = "configurazioni/prodotti/ProdottoListaDati.asp"
        response.redirect virtualPath & "configurazioni/prodotti/ProdottoDatoTecnicoModifica.asp"
        response.end 
     end if 
  End if  
  ...
  if Oper="CALL_DEL_TEC" and CheckTimePageLoad() then 
     ...
     if Cdbl("0" & KK ) > 0 then 
	    q = ""
		q = q & "delete from ProdottoDatoTecnico"
        q = q & " Where IdProdotto = " & IdProdotto
  ...
  qProd = "select * from Prodotto where idProdotto=" & IdProdotto
  IdAnagServizio         = LeggiCampo(qProd,"IdAnagServizio")
  DescProdotto           = LeggiCampo(qProd,"DescProdotto")
...
MySql = MySql & " ,isNull(B.Rigo,9999) as Rigo"
MySql = MySql & " From DatoTecnico A inner join ProdottoDatoTecnico B "
