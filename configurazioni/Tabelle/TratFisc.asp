<%
  NomePagina="TratFisc.asp"
  titolo="Anagrafica Trattamenti Fiscali"
%>
...
if Oper="CALL_INS" or Oper="CALL_UPD" or Oper="CALL_DEL" then 
	...
	Session("swap_IdTrattamentoFiscale")   = KK
	Session("swap_OperTabella")   = Oper
    Session("swap_PaginaReturn")  = "configurazioni/tabelle/TratFisc.asp"
    response.redirect virtualPath & "configurazioni/tabelle/TratFiscDettaglio.asp"
    response.end 
End if 
if Oper="CALL_DETT" then 
	...
	if Cdbl("0" & KK ) > 0 then 
		...
		Session("swap_OperTabella")   = Oper
		Session("swap_PaginaReturn")  = "configurazioni/tabelle/TratFisc.asp"
		response.redirect virtualPath & "configurazioni//TratFisc/TratFiscDettaglio.asp"
    response.end 
	end if 
End if 
						...
						<div class="card">
							<form name="Fdati" Action="<%=NomePagina%>" method="post">
								<div class="card-body d-flex">	
									...
									<h2 style="height: 25px; margin-left: 1rem;">Elenco Trattamenti Fiscali</h2>
								</div>
								<div class="card-body">
									...
									MySql = "" 
									MySql = MySql & " Select * "
									MySql = MySql & " From TrattamentoFiscale "
									MySql = MySql & " WHERE DescTrattamentoFiscale LIKE '%" & apici(cerca_testo) & "%' "
									MySql = MySql & " order By DescTrattamentoFiscale"
										...
										<table class="table">
											...
											if MsgNoData="" then 
												...
												Do While Not rs.EOF and (NumRec<PageSize or Pagesize<=0)
													Primo=Primo+1
													NumRec=NumRec+1
													Id=Rs("IdTrattamentoFiscale")
													err.clear 
											%>
		
											<tr>
												<td>
													<input class="form-control" type="text" readonly value="<%=Rs("DescTrattamentoFiscale")%>">
												</td>
												<td>
