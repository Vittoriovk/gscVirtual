<%
  NomePagina="UtenteCollModifica.asp"
  titolo="Back Office "
   ...
   if Oper=ucase("update") then 
      ...
      if Cdbl(IdUtente)=0 then 
		 if Cdbl(IdAccount)=0 then 
		    ...
		    ConnMsde.execute "Update Account Set IdAzienda=" & Session("IdAziendaWork") & ",IdTipoAccount='BackOColl',FlagAttivo='N',Abilitato=1 Where IdAccount=" & IdAccount
            MyQ = "" 
            MyQ = MyQ & " Insert into Utente (IdAccount,IdAzienda,DescUtente,Cognome,Nome,Email,IdAccountLivello1)"
            MyQ = MyQ & " values (" & IdAccount & "," & Session("IdAziendaWork") & ",'" & Apici(Nominativo) & "'"
            MyQ = MyQ & ",'" & apici(Cognome) & "','" & Apici(Nome) & "','" & apici(Email) & "'," & Session("LoginIdAccount") & ")"
