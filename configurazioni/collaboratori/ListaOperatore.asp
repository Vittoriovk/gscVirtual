<%
  NomePagina="ListaOperatore.asp"
  titolo="Menu - Utenti Azienda"
%>
...
if PaginaReturn="" then 
   PaginaReturn="link/AdminOperatori.asp"
end if 
...
if Oper="CALL_INS" or Oper="CALL_UPD" then
   ...
   response.redirect RitornaA("configurazioni/Collaboratori/UtenteModifica.asp")
   response.end 
...
if Oper="CALL_PRO" then 
   ...
   if Cdbl(IdAccount)>0 then 
	  Session("swap_IdAccount") = IdAccount
      Session("swap_PaginaReturn")    = "configurazioni/Collaboratori/" & NomePagina
      response.redirect RitornaA("configurazioni/prodotti/ProdottiAccountBackO.asp")
      response.end 
   end if    
	...
	%>	
		...
		<div class="container-fluid">
			...
			<div class="row">
				...
				RiferimentoA="col-1 text-center;" & VirtualPath & "link/AdminOperatori.asp" & ";;2;prev;Indietro;;"
				%>
				<!--#include virtual="/gscVirtual/include/Anchor.asp"-->			
				
				<div class="col-11"><h3>Lista Utenti</h3>
				</div>
			</div>
			<%
			AddRow=true
...
<%
			...
			MySql = MySql & " and   a.IdAccountLivello1 = 0"
			if v_Cessati = "" then
		...
		%>
			...
			<td>
			    <% if Rs("FlagAttivo")<>"C" then %>
					...
					<!--#include virtual="/gscVirtual/include/Anchor.asp"-->
				    <%RiferimentoA="col-2;#;;2;prod;Prodotti;;AttivaFunzione('CALL_PRO','" & Rs("IdAccount") & "');N"%>
				    <!--#include virtual="/gscVirtual/include/Anchor.asp"-->											
