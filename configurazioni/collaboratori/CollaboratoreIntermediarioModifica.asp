<%
  NomePagina="CollaboratoreIntermediarioModifica.asp"
...
<%
  ...
  if FirstLoad then 
   ...
   if session("LivelloAccount")=1 then 
      IdAccountLivello1 = Session("LoginIdAccount")
   else 
      IdAccountLivello2 = Session("LoginIdAccount")
   end if 
   ...
   if Oper=ucase("update") then 
      ...
      if Cdbl(IdCollaboratore)=0 then 
		 if Cdbl(IdAccount)=0 then 
            ...
            MyQ = "" 
            MyQ = MyQ & " Insert into Collaboratore (IdAccount,IdAzienda,Denominazione,IdTipoDitta,Livello,IdAccountLivello1,IdAccountLivello2)"
            MyQ = MyQ & " values (" & IdAccount & "," & Session("IdAziendaWork") & ",'" & Apici(Nominativo) & "'"
            MyQ = MyQ & ",'" & apici(IdTipoPers) & "'," & NextL & "," & IdAccountLivello1 & "," & IdAccountLivello2 & ")"
