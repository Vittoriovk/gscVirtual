<%
  NomePagina="ClienteCoobbligatiMod.asp"
  titolo="Modifica Coobligato per cliente"
...
<%
  ...
  IdCoobbligato=0
  if FirstLoad then 
     IdCoobbligato      = "0" & Session("swap_IdCoobbligato")
     if Cdbl(IdCoobbligato)=0 then 
        IdCoobbligato   = cdbl("0" & getValueOfDic(Pagedic,"IdCoobbligato"))
     end if   
     ...
     IdCoobbligato    = "0" & getValueOfDic(Pagedic,"IdCoobbligato")
   ...
   IdCoobbligato = cdbl(IdCoobbligato)
   response.write Session("swap_OperTabella")
   if OperAmmesse="" then 
      if IdCoobbligato = 0 then 
         OperAmmesse="CRUD"
      end if 
   end if 
  ...
  'inserisco account 
   if Oper=ucase("update") then 
      ...
      if Cdbl(IdCoobbligato)=0 then 
         MyQ = MyQ & " insert into AccountCoobbligato"
		 ...
		 if Err.Number=0 then 
		    IdCoobbligato=GetTableIdentity("AccountCoobbligato")
		 end if 
	  end if 
      if Cdbl(IdCoobbligato)>0 then 
	     MyQ = ""
         MyQ = MyQ & " update AccountCoobbligato set "
   ...
   if cdbl(IdCoobbligato)>0 then
      MySql = ""
      MySql = MySql & " Select * From  AccountCoobbligato "
      MySql = MySql & " Where IdAccountCoobbligato=" & IdCoobbligato
	  ...
	  if Cdbl("0" & Getdiz(DizDatabase,"FlagValidato"))=1 or (idStatoValidazione<>"" and idStatoValidazione<>"DOCU") then 
	     OperAmmesse="R"
	  end if 
   end if 
   ...
   elseIf cdbl(IdCoobbligato)=0 then 
      DescPageOper = "Inserimento"
   end if
  'registro i dati della pagina 
   xx=setValueOfDic(Pagedic,"IdCoobbligato"    ,IdCoobbligato)
...
<div class="d-flex" id="wrapper">
    ...
    %>    
        ...
        <div class="container-fluid">
            ...
            <!--#include virtual="/gscVirtual/include/Anchor.asp"-->            
                <div class="col-11"><h3>Gestione Coobbligato :</b> <%=DescPageOper%> </h3>
                </div>
            </div>
   ...
   <!--#include virtual="/gscVirtual/include/showErrorDivRow.asp"-->
    ...
    <%
      ...
      ReadOnly=""
      SoloLettura=false
      if instr(OperAmmesse,"U")=0 or (instr(OperAmmesse,"I")>0 and cdbl("0" & IdCoobbligato)>0) then 
         SoloLettura=true
         ReadOnly=" readonly "
      end if 
