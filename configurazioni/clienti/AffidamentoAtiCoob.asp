<%
  NomePagina="AffidamentoAtiCoob.asp"
...
if qSelDitta<>"" then 
	...
	if cdbl(FlagValidato)=0 or cdbl(ValidoAl) < Cdbl(Dtos()) then 
	   ...
	   if IdStatoValidazione<>"" then 
		    ...
		    Stato = trim(DescStatoValidazione & " " &  NoteValidazione)
		end if 
	else
	     Stato = "Validato"
	end if 	
...
if TipoRife = "COOB" then 
   InfoH="Affidamento coobbligato"
else
   InfoH="Affidamento ATI"
end if 
%>
			...
			<div class="row">
                ...
                <div class="row">
				   ...
				   <div class = "col-2"> 
                        ...
                        <input value="<%=Stato%>" readonly class="form-control"  >
                        </div>
				   </div>
				   <div class = "col-1"> 
                        <div class="form-group ">
                        <%xx=ShowLabel("Valido Dal")%>
                        <input value="<%=Stod(ValidoDal)%>" readonly class="form-control"  >
                        </div>
				   </div>				   
				   <div class = "col-1"> 
                        <div class="form-group ">
                        <%xx=ShowLabel("Valido Dal")%>
                        <input value="<%=STod(ValidoAl)%>" readonly class="form-control"  >
                        </div>
				   </div>					   
              </div>			
			<%
...
'caricamento tabella 
   ...
   if FirstLoad and (TipoRife="COOB" or TipoRife="ATI") then 
      ...
      if Vuoto="" then 
		 ...
		 MyQ = MyQ & " and (DITT='" & IdTipoDitta & "' or PEGC='" & IdTipoDitta & "' or PEGI='" & IdTipoDitta & "' or PEFI='" & IdTipoDitta & "')"
		 'response.write MyQ
