...
function LMPS_CheckPagamento(idAccCli,idAccReq)
{
   ...
   if (idAccReq>0) {
      ...
      if (whichRE.includes("|ESTE|"))
         reqEste = IsChecked(prefix + "ESTE_" + idAccReq);		 
   }
   ...
   if (idAccReq>0 && reqBors==false && reqFido==false && reqEstr==false && reqEste==false) {
...
'IdAccountLivello1      = Account 1* livello  : per gestire la modalita' di gestione
'IdProcessoElaborativo  = processo elaborativo: default STANDARD
'   STANDARD   : pagano tutti dal cliente al livello 1 
'              : in tal caso al pagamento viene messo in Pagato (PAGA)
'output 
...
IdAccountLivello2 = "0" & LeggiCampo("select * from Cliente Where IdAccount=" & IdAccountModPagCliente,"IdAccountLivello2")
...
flagProcessoAlt  = false 
flagPagamColl    = true 
flagCollOnlyBors = false 

if IdProcessoElaborativo<>"STANDARD"  then 
   flagProcessoAlt = true 
end if 
...
%>
   ...
   <div class="table-responsive"><table class="table"><tbody>
	  ...
	  'se non sono cliente e ho dettagli metto il riferimento per cliente 
	  if (FlagBorsellinoR=1 or FlagFidoR=1 or FlagEstrattoR=1 or FlagEsternoR=1) and tmpIdAccountRequest>0 then 
	  %>
	  <tr>
	      <td colspan="7" scope="col">
			   <div class="bg-primary text-center text-white font-weight-bold">
	  ...
	  if FlagEsternoR=1 then
	     LMP_opt="Pagamento esterno"
	     TipoPaga = "ESTE"
		 CodePaga = tmpIdAccountRequest
	     ImptTota = 0
		 ImptImpe = 0
